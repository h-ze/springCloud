package com.qdone.support.rate.redisson.aspect;

import com.alibaba.fastjson.JSONObject;
import com.qdone.support.rate.redisson.anontation.RateLimiter;
import com.qdone.support.rate.redisson.exception.RateLimitException;
...
public class RedissonRateAspect {
    ...
    @Pointcut("@annotation(com.qdone.support.rate.redisson.anontation.RateLimiter)")
    ...
    public Object doRedissonRateAround(ProceedingJoinPoint point) throws Throwable {
        ...
        try {
            ...
            if (!ObjectUtils.isEmpty(rateLimiter)) {
                key = REDISSON_RATE_LIMITER_PRIEX+parse(rateLimiter.value(),rateLimiter.useArgs(), method, point.getArgs(), point);
    ...
    private String parse(String key,Boolean useArgs,Method method, Object[] args, ProceedingJoinPoint point)  {
        if(!ObjectUtils.isEmpty(useArgs)&&useArgs){
